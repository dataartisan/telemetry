...
try:
    with open('/opt/telemetry/config/userconfig.yaml','r') as cfile:
...
try:
    with open('/opt/telemetry/telegraf/telegraf.conf.temp','r') as tfile:
...
for host in config_data['telegraf']['sql_plugin']['hosts']:
    conn_strings += '    "Server={};Port={};User Id={};Password={};app name=LnW-Telemetry;",\r\n'.format(host['host'],host['port'],host['username'],host['password'])

# Write the new telegraf config
try:
    with open('/opt/telemetry/telegraf/telegraf.conf','w') as tconf:
